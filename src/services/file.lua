...
local function CreatePayloadFileObject(optType, optPath, bufferSize)
    local File = {}
...
local function CreatePayloadFolderObject(optPath)
    local Folder = {}
...
local function CreateDiskFileObject(optType, optPath, bufferSize)
    local File = {}
...
local function CreateDiskFolderObject(optPath)
    local Folder = {}
    ...
    function Folder:Open()
        ...
        local openFolderCount = module:GetOpenFolderCount()
        if openFolderCount > module.MAX_SAFE_FOLDERS then
            print(tostring(openFolderCount),
                "folders are opened in memory! You may not be deloading certain folders properly!")
            print("Recommended concurrent folder count (FileService.MAX_SAFE_FOLDERS):", module.MAX_SAFE_FOLDERS)
        end
    end
...
-- PAYLOAD FUNCTIONS
function module:ReadPayloadFile(filePath, openType)
    local fileObject = CreateFileObject(openType, filePath)
    fileObject:Open()
    return fileObject
end
function module:PreparePayloadFile(openType)
    local fileObject = CreateFileObject(openType)
    return fileObject
end

function module:ReadPayloadFolder(folderPath)
    local folderObject = CreateFolderObject(folderPath)
    folderObject:Open()
    return folderObject
end
function module:PreparePayloadFolder()
